...
def validPassports():
	...
	for _ in range(989):
		k = input()
		if not k:
			if 'byr:' in inp and 'iyr:' in inp and 'eyr:' in inp and 'hgt:' in inp and 'hcl:' in inp and 'ecl:' in inp and 'pid:' in inp:
				cnt+=1
				lst.append(inp)
			inp = ''
		else:
	...
	for i in lst:
		...
		for k in p:
			...
			ok = k.split(':')
			if ok[0]=='byr':
				if int(ok[1]) in range(1920,2003):
					print()
				else:
					flag = False
			elif ok[0]=='iyr':
				if int(ok[1]) in range(2010,2021):
					print()
				else:
					flag = False
			elif ok[0]=='eyr':
				if int(ok[1]) in range(2020,2031):
					print()
				else:
					flag = False
			elif ok[0]=='hgt':
				if ok[1].endswith('cm'):
					if int(ok[1][:-2]) in range(150,194):
						print()
					else:
						flag = False
				elif ok[1].endswith('in'):
					if int(ok[1][:-2]) in range(59,77):
						print()
				...
				if ok[1][0] =='#' and len(ok[1])==7:
					for c in ok[1][1:]:
						if c in letters[:6] or c.isdigit():
							print()
						else:
							flag = False
				else:
				...
				if ok[1] in ['amb','blu','brn','gry','grn','hzl','oth']:
					print()
				else:
					flag = False
			elif ok[0]=='pid':
				if len(ok[1])==9 and ok[1].isdigit():
					print()
				else:
					flag = False
		if flag ==True:
