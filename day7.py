...
def countBags():
	d = {}
	for _ in range(594):
		m =  re.match('([a-z]* [a-z]*) bags contain ([a-z ,1-9]*).', input())
		print(m.group(1),m.group(2))
		if m.group(2)!='no other bags':
			key = ''.join(m.group(1).split())
			d[key] = m.group(2).split()
			temp = []
			for i,k in enumerate(d[key]):
				ok = k.strip(',')
				print(ok)
				if ok!='bag' and ok!='bags':
					temp.append(ok)
				if i==len(d[key])-1:
					d[key] = temp
	print(d.keys())
	...
	if a in value1:
		...
		for j,k in enumerate(d[a]):
			if k not in range(1,10):
				t = fn(k,d[a][j-1],value)
				temp+=t*d[a][j-1]
		value1[a]=temp+1
		return temp+1
def countBagsInShinyGold():
	for _ in range(594):
		m =  re.match('([a-z]* [a-z]*) bags contain ([a-z ,1-9]*).', input())
		if m.group(2)!='no other bags':
			key = ''.join(m.group(1).split())
	...
	for k in d:
		...
		for i in d[k]:
			...
			cnt+=1
		d[k] = t
	print(fn("shinygold",1,value1))
countBagsInShinyGold()
