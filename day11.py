...
def cpyList(inp):
	out = [['' for _ in range(91)] for _ in range(90)]
	for i in range(90):
		for j in range(91):
			out[i][j] = inp[i][j]
	return out
	...
	while True:
		...
		for i in range(90):
			for j in range(91):
				...
				if i==0:
					if j==0:
						adj = [inp[ i ][ j   +1  ], inp[ i  + 1   ][ j  ], inp[ i + 1 ][ j + 1 ]]
					elif j==90:
						adj = [inp[ i  ][ j  -1  ], inp[ i  + 1   ][ j  ], inp[ i + 1 ][ j - 1 ]]
					else:
						adj = [inp[ i ][ j   +1  ], inp[ i  + 1   ][ j  ], inp[ i + 1 ][ j + 1 ], inp[ i  ][ j  -1  ], inp[ i + 1 ][ j - 1 ]]
				elif i==89:
					if j==0:
						adj = [inp[ i ][ j   +1  ], inp[ i - 1 ][ j], inp[ i - 1 ][ j + 1 ]]
					elif j==90:
					...
					if j==0:
						adj = [inp[ i ][ j   +1  ], inp[ i - 1 ][ j], inp[ i - 1 ][ j + 1 ] , inp[ i  + 1   ][ j  ], inp[ i + 1 ][ j + 1 ]]
					elif j==90:
						adj = [inp[ i  ][ j  -1  ], inp[ i - 1 ][ j], inp[ i - 1 ][ j - 1 ], inp[ i  + 1   ][ j  ], inp[ i + 1 ][ j - 1 ]]
					else:
						adj = [inp[ i ][ j   +1  ], inp[ i - 1 ][ j], inp[ i - 1 ][ j + 1 ] , inp[ i  + 1   ][ j  ], inp[ i + 1 ][ j + 1 ], inp[ i - 1 ][ j - 1 ], inp[ i + 1 ][ j - 1 ], inp[ i  ][ j  -1  ]]
				if inp[i][j]=='L':
					if adj.count('#')==0:
						out[i][j] = '#' 
				elif inp[i][j]=='#':
