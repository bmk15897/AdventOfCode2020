...
# DAY 2
def getValidPasswordsCount():
	cnt = 0
	for _ in range(1000):
		m =  re.match('(\d*)-(\d*) ([a-z]): ([a-z]*)', input())
		#if m.group(4).count(m.group(3)) in range(int(m.group(1)),int(m.group(2))+1):
		#	cnt+=1
		#	print("ok")
		
		if(m.group(4)[int(m.group(1))-1]==m.group(3) and m.group(4)[int(m.group(2))-1]!=m.group(3)) or (m.group(4)[int(m.group(2))-1]==m.group(3) and m.group(4)[int(m.group(1))-1]!=m.group(3)):
			cnt+=1
	print(cnt)
