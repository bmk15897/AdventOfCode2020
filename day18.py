	...
	for n,i in enumerate(s):
		if i == '(':
			...
			while k!='(':
				ok.append(k)
				k=stack.pop()
			stack.append(str(solveSimpleEx2(ok[::-1])))
		elif i!=' ':
			print(i)
			stack.append(i)
	return stack
		
def solveSimpleEx(inp):
	prev = None
	prevOp = ''
...
def solveSimpleEx2(inp):
	...
	for i in range(len(inp)):
		...
		if t == '+':
			p = ok.pop()
			ok.append(str(int(p)+int(inp[i+1])))
			skip = i+1
		else:
			ok.append(t)
	return solveSimpleEx(ok)

def solve1():
	ans = 0
	for _ in range(377):
		inp = input()
		ans+=solveSimpleEx2(resolveParanetheses(inp))
	print(ans)
